 ...
 *
 * The server is started lazily on first use (session_start) and restarted
 * automatically if it crashes.
 *
 * Placement: .pi/extensions/quarkus-mcp/index.ts  (project-local)
 */
...
function jbangBin(): string {
  ...
  // We try common install directories; spawn will fail fast if none work.
  const candidates = [
...
export default async function (pi: ExtensionAPI) {
  ...
  // -------------------------------------------------------------------------
  // /quarkus-mcp command – manual reload / status
  // -------------------------------------------------------------------------
  ...
  pi.registerCommand("quarkus-mcp", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!client) {
        ctx.ui.notify("quarkus-mcp: server not running. Use /quarkus-mcp restart.", "warning");
        return;
      }
